...
function parseNoteFromRaw(
  ...
  const year = d.year ? (Array.isArray(d.year) ? d.year : [d.year]) : undefined;
...
function loadNotesFromGithub(
  ...
  return Effect.gen(function* () {
    ...
    const blobs = yield* Effect.tryPromise({
      ...
      catch: (e) => new E.NotesError({ message: `Failed to fetch note contents: ${String(e)}` }),
    });
