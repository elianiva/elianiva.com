      ...
      >
        ...
        <div className="flex items-start justify-between gap-3 md:hidden">
          <div className="flex-1 min-w-0">
            <h3 className="font-bold font-display text-pink-950 leading-tight">
              {company}
            </h3>
          ...
          <div className={"text-pink-600 transition-transform duration-200 shrink-0 mt-0.5 " + (isOpen ? "rotate-180" : "")}>
        ...
        <div className="hidden md:flex md:flex-row items-start md:justify-between gap-4 -ml-8">
          <div className="flex items-start gap-3">
            <div className={"text-pink-600 transition-transform duration-200 shrink-0 " + (isOpen ? "rotate-180" : "")}>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-row items-center gap-3">
                <h3 className="font-bold font-display text-pink-950 whitespace-nowrap">{company}</h3>
                <div className="flex items-center gap-2">
                  <span className="text-xs px-3 py-1 bg-pink-100/60 font-mono uppercase whitespace-nowrap">{location}</span>
                  <span className="text-xs px-3 py-1 bg-pink-100/60 font-mono uppercase whitespace-nowrap">{time}</span>
                </div>
              </div>
        ...
        <div ref={setInitialStyles} className="overflow-hidden">
          <ul className="list-disc list-outside pl-4 mt-2">
            {details.map((detail, i) => (
              <li key={i} className="text-sm md:text-base leading-relaxed font-body text-pink-950/80">{detail}</li>
            ))}
          </ul>
          <ul className="flex flex-wrap items-center pt-2 gap-1">
            {technologies.map((technology) => (
              <li key={technology} className="font-mono uppercase text-pink-950 text-xs border border-dashed border-pink-400/60 py-1 px-3 stitch-border">{technology}</li>
            ))}
          </ul>
        </div>
      </button>
