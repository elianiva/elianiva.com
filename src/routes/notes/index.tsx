...
function NotesPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      <BackButton />
      <div className="mb-4 pt-6"><h1 className="font-display text-3xl md:text-4xl font-bold text-pink-950 tracking-wide">Personal Notes Vault</h1><p className="text-pink-950/70 mt-2 font-body">A collection of notes, articles, and thoughts. These are pulled from my personal obsidian vault.<br />There are <span className="text-pink-500">{notes.length}</span> public notes.</p></div>
      {notes.length > 0 && <div className="mb-4"><Search notes={notes} onSearch={handleSearch} /></div>}
      {notes.length === 0 ? (<div className="text-center py-20 text-pink-950/50"><p>No public notes yet.</p><p className="text-sm mt-2"><code>public</code> tag to publish them.</p></div>) : (<NotesTabs notes={displayedNotes} />)}
      {hasGraphData && (<div className="mt-8 flex justify-center"><button onClick={() => setGraphOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-white/60 border border-pink-200 text-sm font-body text-pink-950 hover:bg-white hover:shadow-card transition-all"><GraphIcon className="w-4 h-4" />View Graph</button></div>)}
      <GraphModal graph={graph} isOpen={graphOpen} onClose={() => setGraphOpen(false)} onNodeClick={(slug) => { window.location.href = `/notes/${slug}`; }} />
    </div>
  );
}