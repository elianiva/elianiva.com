...
import { TermPopover } from "~/components/term-popover.tsx";

export const revisionDef =
  "A revision is Jujutsu's equivalent of a commit, but unlike Git commits, they remain mutable until pushed to a remote.";
export const bookmarkDef =
  "Bookmarks are Jujutsu's equivalent of Git branches. They're named pointers to revisions that move when you create new descendants.";
export const parentDef =
  "A parent revision is the ancestor that a revision is based on. Revisions form a tree structure where each can have one or more parents.";
export const descendantDef =
  "Descendants are revisions that come after a given revision in the tree. Changes to a parent automatically cascade to all its descendants.";
export const changeDef =
  "In Jujutsu, changes represent your uncommitted modifications. Unlike Git's staging area, all changes are automatically tracked and snapshotted.";
export const stagingAreaDef =
  "The staging area (or index) is Git's way of preparing changes before committing. Jujutsu removes this concept entirely. All changes are automatically tracked.";
...
One gotcha you might not be used to if you're used to Git is that you have to move the <TermPopover client:visible term="bookmark" definition={bookmarkDef} /> _with_ you. Whenever you start a new <TermPopover client:visible term="revision" definition={revisionDef} />, that bookmark stays, unlike a Git branch which sticks to the latest commit.
...
import { RegexHighlighter } from "~/components/regex-highlighter.tsx";
...
<Quiz
  ...
  options={[
    ...
    {
      id: "c",
      text: "HEAD@{14} - The last commit before rebase operations began",
      isCorrect: true,
    },
  ...
  correctResponse={{
    title: "Correct!",
    message:
      "The reflog shows <code>HEAD@{14}</code> contains the last commit before the rebase started. The entry <code>c1805f1</code> represents the safe state of your feature branch before things went sideways.",
  }}
  errorResponse={{
    title: "Not quite!",
    message:
      "While those entries look tempting, they contain the messy rebase operations. Look for the last entry before the rebase started, that's where your safe state lives.",
  }}
/>

Congratulations if you got it right! In all fairness, it's not _that_ hard to begin with, there's just a lot of noise that needs to be filtered out to get where we want. Alright, so now we know the HEAD that we wanted, the next step is to do `git reset --hard HEAD@{N}` right? A destructive command that permanently discards everything. Let's just assume you get the number correct, problem solved, but what if you didn't? What if you did a typo and you didn't realise? What if you got the number wrong? Then you're left with an incorrect reset, back to the broken state we go! Oh, and as a bonus, all of your unstaged files are gone.
...
I know it's probably not _that_ big of a deal for some people, but it's just very annoying having to figure out all these numbers when all you want to do is just undo your last action. Also the fact that you have to rely on a destructive operation doesn't sound great to me, we're all bound to make some mistakes, it would be nice if there's a safeguard that catches me.

### Ol' Trusty Ctrl+Z

In Jujutsu, you can just do `jj undo` to undo your last action, and you can _undo_ the _undo_! Instead of reflog that shows all the internal state transitions, we have `jj op log`, which is more intuitive and more human readable. Take a look at this:
...
<RegexHighlighter
  patterns={[
    ...
    {
      pattern:
        "^(│  )(new empty commit|describe commit|snapshot working copy|rebase commit|point bookmark)",
      color: "#618774",
    },
...
It's one of those things that you _need_ to try it yourself for quite some time for you to feel its benefit. I haven't used Git for a few months now, and whenever I get back I miss Jujutsu's working copy and <TermPopover client:visible term="revision" definition={revisionDef} /> abstraction.
