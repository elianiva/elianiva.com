...
>()("KvCache") {
  static readonly layer = Layer.effect(
    ...
    Effect.sync(() => {
      ...
      const getOrSet = Effect.fn("KvCache.getOrSet")(function* <A, E, R>(
        key: string,
        ttl: Duration.Input,
        load: Effect.Effect<A, E, R>,
      ) {
        if (ns) {
          ...
          if (raw) {
            ...
            yield* Effect.tryPromise(() => ns.delete(kvKey(key))).pipe(
              Effect.catchCause(() =>
                Effect.sync(() => console.error(`[KvCache] failed to delete key: ${key}`)),
              ),
            );
          }
        }
        ...
        if (ns) {
          ...
          yield* Effect.tryPromise(() =>
            ...
            Effect.catchCause(() =>
              Effect.sync(() => console.error(`[KvCache] failed to put key: ${key}`)),
            ),
          );
        }
        return value;
      ...
      const invalidate = Effect.fn("KvCache.invalidate")(function* (key: string) {
        if (ns) {
          yield* Effect.tryPromise({
